...
### 协程
协程是在用户空间利用程序语言的语法语义就能实现逻辑上类似多任务的编程技巧，由程序员自己调度运行的并行计算。一个线程可以创建任意数量的协程，协程的切换不会引起线程或者进程的切换。线程是由系统调度的，线程切换或线程阻塞的开销都比较大。
...
### 内核线程和用户线程
内核线程，在邮递系统上也称为轻量性进程（Light Weight Process, LWP），运行在内核空间，用内核来调度；用户线程运行在用户空间，由线程库来调度。

完全在用户空间实现的线程无需内核的支持，内核甚至根本不知道这些线程的存在。线程库负责管理所有执行线程，比如线程的优先级、时间片等。线程库利用 longjmp 来切换线程的执行，使他们看起来像是“并发”执行的。但实际上内核仍然是把整个进程作为最小单位来调度。也就是说，一个进程的所有执行线程共享该进程的时间片，他们对外表现出相同的优先级。完全在用户空间实现的线程的优点是：创建和调度线程都无需内核的干预，因此速度相当快。并且由于它不占用额外的内核资源，所以即使一个进程创建了很多线程，也不会对系统性能造成明显的影响。其缺点是：对于多处理器系统，一个进程的多个线程无法运行在不同的 CPU 上，因为内核是按照其最小调度来分配 CPU 的。此外，线程的优先级只对同一个进程中的线程有效，比较不同进程中的线程的优先级没有意义。

完全由内核调度的模式将创建、调度线程的任务都交给了内核，运行在用户空间的线程无需执行管理任务，这与完全在用户空间实现的线程恰恰相反。二者的优缺点正好互换。

双层调度模式是前两种实现模式的混合体。这种线程实现方式结合了前两种方式的优点：不但不会消耗过多的内核资源，而且线程切换速度也比较快，同时它可以充分利用多处理器的优势。
